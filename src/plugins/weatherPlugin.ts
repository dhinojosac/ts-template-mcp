...
export function registerWeatherTools(server: McpServer) {
  ...
  server.registerTool(
    ...
    async (args: { [x: string]: any }) => {
      const { latitude, longitude }: WeatherForecastArgs = validateToolArgs(WeatherForecastSchema, args);
  ...
  server.registerTool(
    ...
    async (args: { [x: string]: any }) => {
      const { state }: WeatherAlertsArgs = validateToolArgs(WeatherAlertsSchema, args);
      
      // Simulate alerts API call (replace with actual API)
      const alerts = await simulateAlertsAPI(state);
 ...
 */
async function simulateWeatherAPI(latitude: number, longitude: number): Promise<string> {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 100));
  
  // Generate mock forecast based on coordinates
  const temperature = Math.round(20 + (latitude * 0.5) + (Math.random() * 10 - 5));
 ...
 */
async function simulateAlertsAPI(state: string): Promise<string> {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 100));
...
const weatherPlugin: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{ Params: { state: string } }>('/weather/alerts/:state', {
    schema: {
      params: {
        type: 'object',
        properties: {
    ...
    try {
      // Use the same validation as the MCP tool
      const args = { state };
      const { state: validatedState }: WeatherAlertsArgs = validateToolArgs(WeatherAlertsSchema, args);
      
      const alerts = await simulateAlertsAPI(validatedState);
